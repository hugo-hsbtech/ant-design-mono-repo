 ...
 * the server (from the cookie) — so the first paint already matches and there
 * is no light→dark flash — and persists toggles back to the cookie.
 */
export function Providers({
  ...
  return (
    <ThemeProvider defaultMode={initialMode} onModeChange={persist}>
      {children}
    </ThemeProvider>
  );
}
