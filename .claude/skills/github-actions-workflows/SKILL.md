...
backend/packages/apps/worker/**           → backend-tests, deploy-temporal-worker
backend/packages/apps/document-investigation/** → backend-tests (not yet deployed)
backend/packages/domains/**               → backend-tests, deploy-api, deploy-temporal-worker
...
### Temporal Worker Deploy Pattern

The worker runs as **3 ECS services** (`worker-cpu`, `worker-io`, `worker-sales`) from the same Docker image,
pushed to 3 ECR repos. The `deploy-temporal-worker` job depends on `terraform-apply` (ECR repos must exist first).

**Build dependency**: the runner Dockerfile (`FROM ezra-worker:latest`) requires the worker image to exist locally.
The `build-backend` job builds the worker image (no push) so the runner build can reference it.
The `deploy-temporal-worker` job rebuilds + pushes to all 3 ECR repos.

```yaml
# build-backend: build worker image locally (no push) — runner depends on it
- name: Build worker image
  working-directory: backend
  run: docker build -t ezra-worker:latest -f packages/apps/worker/Dockerfile .

# deploy-temporal-worker: rebuild + push to 3 ECR repos + deploy 3 ECS services
- name: Push worker image to all ECR repos
  run: |
    for repo in ezra-temporal-cpu-worker \
                ezra-temporal-io-worker \
                ezra-temporal-sales-worker; do
      docker tag ezra-worker:latest ${{ steps.ecr.outputs.registry }}/${repo}:${{ github.sha }}
      docker tag ezra-worker:latest ${{ steps.ecr.outputs.registry }}/${repo}:latest
      docker push ${{ steps.ecr.outputs.registry }}/${repo} --all-tags
    done

- name: Deploy temporal workers
  run: |
    for service in ezra-temporal-cpu-worker \
                   ezra-temporal-io-worker \
                   ezra-temporal-sales-worker; do
      aws ecs update-service --cluster ${{ vars.ECS_CLUSTER }} --service "${service}" --force-new-deployment
    done
    for service in ezra-temporal-cpu-worker \
                   ezra-temporal-io-worker \
                   ezra-temporal-sales-worker; do
  ...
  workflow_dispatch:
    inputs:
      service:
        ...
        required: true
        type: choice
        options: [api, deal-triage-frontend, temporal-worker]
      image_tag:
