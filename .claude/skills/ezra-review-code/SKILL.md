...
- [ ] (Frontend) No raw entity IDs rendered as user-facing text — deal/company/contact **names** go through `@ezra/formatting/entityNames` helpers; no `name ?? id` or `` `Deal ${id}` `` fallbacks in text, `aria-label`, `title`, or export columns. IDs only in `key`/`href`/`htmlFor`/`data-*`/payloads.
